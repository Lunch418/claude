...
| S-01 | Проверка ключа SSH-бастиона отключена (`StrictHostKeyChecking=no`) | ✅ исправлено в коде + 🔶 остаточный риск | оба remote-скрипта, см. ниже |
...
`KSP_DB_USER`, `MONITORING_DB_USER`, `LOG_DB_USER`):
...
## S-01 — проверка ключа SSH-бастиона (исправлено + остаточный риск)

Было: `StrictHostKeyChecking=no` + `UserKnownHostsFile=/dev/null` в обоих
remote-скриптах — ключ бастиона не проверялся вообще, ни разу. Через это
же SSH-соединение в открытом виде идут PAM-пароль и креды всех БД
(SED/CHED/CHED2/KSP/АИС Мониторинг) — активный MITM на сетевом пути к
бастиону (ARP-спуфинг в том же VLAN и т.п.) снимал бы все учётки сразу.

Исправлено: `StrictHostKeyChecking=accept-new` + постоянный файл
`storage/pam_known_hosts` (закрыт от HTTP, как весь `storage/`). Ключ
бастиона фиксируется автоматически при первом успешном подключении;
после этого любая подмена ключа (повторный MITM, компрометация
бастиона) обрывает соединение вместо молчаливого доверия новому ключу.

Остаточный риск (организационно, не чинится кодом): TOFU защищает
только от подмены ключа **после** первого удачного коннекта, не в
момент самого первого раза. После первого запуска на бою сверьте
отпечаток:

```bash
ssh-keygen -lf storage/pam_known_hosts
```

с администраторами PAM-бастиона по отдельному доверенному каналу (не
через это же SSH-соединение и не по тому же каналу связи, который мог
быть скомпрометирован). Если меняется сам бастион (переустановка,
смена оборудования) — удалить `storage/pam_known_hosts` и повторить
сверку при следующем подключении.
