...
(function () {
  ...
  // Ищет в тексте запроса конструкции FROM/JOIN <table> [AS] <alias>.
  ...
  function init() {
    ...
    // подсветки и инициализируются ПОЗЖЕ, затирая наш цветной слой.
    // Наблюдатель восстанавливает цветную подсветку при любом затирании.
    try {
    ...
    function suggestions(cw) {
      ...
      if (!ctx.kw) {
        ...
        if (cw.empty && /[A-Za-z_]\w*\s+$/.test(ctx.tail)) {
          // таблица введена, дальше пробел → продолжения
    ...
    function acceptAC() {
      ...
      if (it.k === 'fn') { caret = acStart + (it.raw.length + 1); } 
