...
async function apiCall(sql, mode = 'preview', limit = 1000) {
  const key = sql + '|' + mode + '|' + limit + '|' + (state.currentDb||'remote');
...
function switchDb(db) {
  ...
  document.getElementById('logoDbLabel').textContent = db === 'local' ? '/ log' : '/ doc';
  ...
  if (db === 'local') {
    ...
    if (fkBadge)     { fkBadge.style.visibility = 'hidden'; fkBadge.style.opacity = '0'; }
